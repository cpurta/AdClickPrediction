...
submission = './output/submission_1.csv'    # path of to be outputted submission file
...
output = pd.DataFrame(columns=['display_id','ad_id','likelihood'])

print('Predicting test likelihoods...')

for t, ID, DID, x, y in data(test, D):
    p = learner.predict(x)
    if t % 2500000 == 0 and t > 1:
        print('%s\tPredicted %d test records' % (datetime.now(), t))
    output.append(pd.DataFrame([[DID, ID, p]], columns=['display_id','ad_id','likelihood']))
    

print('Predicted likelihood by ad_id:\n{}'.format(output[:10]))

output.sort_values(['display_id','likelihood'],inplace=True,ascending=False)

print('Sorted ad_ids by likelihood:\n{}'.format(output[:10]))
